...
fn smoke_principal() -> VerifiedPrincipal {
    principal(
        "mvp-real-smoke",
...
fn build_service(
    ...
    store: Arc<SqliteRunPersistence>,
...
async fn report_run(
    ...
    if let Ok(events) = service.read_events(principal, key, None, 128).await {
        for event in events {
            eprintln!(
    ...
    for _ in 0..900 {
        if let Ok(view) = service.get_run_status(principal, key).await {
            if view.disposition == expected {
                report_run(service, principal, key, &view).await;
                return view;
            }
            if matches!(
                view.disposition,
...
async fn real_readonly_knowledge_to_local_model_answer() {
    ...
    let service = Arc::new(build_service(harness, store, vec![workflow]));
    let principal = smoke_principal();
    ...
    let view = wait_for(&service, &principal, key, RunDispositionV1::Completed).await;
...
async fn real_localwrite(decision: ApprovalDecisionV1) {
    ...
    let service = Arc::new(build_service(harness, store.clone(), vec![workflow]));
    let principal = smoke_principal();
    let session = service.create_session(&principal).await.expect("session");
    let prompt = format!(
    ...
    let run = service
        ...
        .expect("start");
    let key = RunKey::new(run.run_id, session);
    wait_for(&service, &principal, key, RunDispositionV1::Waiting).await;
    let (waits, _) = service
        .waiting_page(&principal, None, 8)
        .await
        .expect("waiting");
    let wait = waits.first().expect("wait").clone();
    let preview = service
        .approval_preview(&principal, key, wait.wait_id)
        .await
        .expect("preview");
    ...
    service
        .submit_decision(&principal, key, wait.wait_id, wait.row_version, decision)
        .await
        .expect("decision");
    service
        .resume_run(&principal, key, Some(wait.wait_id))
        .await
        .expect("resume");
    ...
    if decision == ApprovalDecisionV1::Approve {
        ...
        let view = wait_for(&service, &principal, key, RunDispositionV1::Failed).await;
