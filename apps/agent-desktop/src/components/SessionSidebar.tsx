...
export function SessionSidebar({ expanded, conversations, selectedSessionId, onSelectSession, onNewTask }: Props) {
  if (!expanded) {
    return (
          ...
          <nav aria-label="Recent conversations" className="space-y-0.5">
            {conversations.map((conversation) => (
              <button key={conversation.sessionId} type="button" className={cn("group w-full rounded-control px-2 py-2 text-left outline-none transition-colors hover:bg-selection focus-visible:ring-2 focus-visible:ring-focus", selectedSessionId === conversation.sessionId && "bg-selection")} aria-current={selectedSessionId === conversation.sessionId ? "page" : undefined} onClick={() => onSelectSession(conversation.sessionId)}>
                <span className="flex items-center gap-1.5"><span className="block min-w-0 flex-1 truncate text-xs font-medium">{conversation.title}</span>{conversation.activeRun !== null && <LoaderCircle aria-label="Run active" className="size-3 shrink-0 animate-spin text-accent" />}</span>
                <span className="mt-0.5 block truncate text-[11px] text-muted">{conversation.activeRun?.activity ?? (conversation.lastRun?.outcome === "completed" ? "Completed" : "Conversation")}</span>
              </button>
            ))}
          </nav>
        )}
