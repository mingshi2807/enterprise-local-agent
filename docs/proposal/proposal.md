...
# M5 Architecture Proposal — Model-Driven Typed Action Planning

## Summary

  M5 will use a strict provider-neutral JSON envelope carried by exactly one textual ModelOutputPart. The harness will decode and validate the untrusted proposal, generate all correlation IDs, audit metadata-
  only progression, and produce an opaque ValidatedAction. Act will consume that value through LoopEffects, while ExecutionHarness::invoke_tool remains the sole policy, budget, audit, and execution authority.

  No new crate is justified. LoopEngine, RigModelAdapter, and the M4 provider path remain unchanged.

  Current M0–M4 baseline is clean and all 105 tests plus two compile-fail doctests pass.

## 1. Proposed tree

  crates/agent-core/src/
  ├── action.rs                  # proposal vocabulary and rejection metadata
  ├── ids.rs                     # ActionProposalId
  ├── event.rs                   # metadata-only action events; schema v4
  └── tool.rs                    # ToolResult::call_id correlation accessor

  crates/agent-harness/src/
  ├── action.rs                  # strict decoder, schema subset, validator, opaque ValidatedAction
  ├── action_tests.rs
  ├── execution.rs               # tracked model response; prepare/invoke validated action
  ├── registry.rs                # schema compilation during registration
  ├── error.rs                   # new action-related harness operations
  └── testing.rs                 # fake tool behavior that echoes the runtime ToolCallId

  crates/agent-loop/src/
  └── program.rs                 # narrow governed action methods on LoopEffects

  apps/agent-cli/src/
  ├── main.rs                    # composition and safe metadata output
  └── action_program.rs          # concrete M5 LoopProgram and WorkingState

  README.md                      # deterministic and optional M4-live demonstration

  No gateway, action-validation, or provider-specific crate will be added.

## 2. Action domain model

  In agent-core:

  pub struct ActionProposalId(Uuid);
...
- ActionProposalId is a distinct UUID newtype generated by trusted runtime code after a governed model invocation.
- It is never read from model output and cannot be confused with ToolCallId.
- ActionProposal is provider-neutral shared vocabulary, but is not a ToolCall, executable capability, or authorization.
- Proposal arguments reuse the already redacted ToolInput; the enclosing proposal receives a custom redacted Debug and no persistence-specific API.
- ValidatedAction belongs in agent-harness. It has no public constructor, no serialization, no clone, and no raw argument accessor. Act consumes it by value.
- ActionValidationError belongs in agent-harness and exposes only proposal_id() and a stable reason().

  LoopState, VerificationResult, and action-specific working state remain outside core.

## 3. Parser/decoder

  M5 selects option A: a strict textual JSON action envelope.

  Options B and C remain deferred because native tool calls require provider correlation and continuation semantics, while structured-output configuration would change the provider request contract.
...
- ModelResponse must contain exactly one ModelOutputPart::Text.
- Zero, multiple, mixed, or ModelOutputPart::ToolCall outputs are rejected atomically.
- The entire text must be one JSON document. Leading/trailing JSON whitespace is accepted; content is otherwise not trimmed or rewritten.
- Top level must contain exactly action.
- action must contain exactly tool and arguments.
- arguments must be a JSON object.
- Unknown and duplicate fields are rejected at every object level, including recursively inside arguments.
- A custom Serde visitor will detect duplicate object keys; this is strict decoding, not a home-grown schema validator.
- Missing fields, code fences, prose, trailing content, fuzzy tool names, case normalization, and markdown extraction are rejected.
- The decoder generates ActionProposalId before parsing so malformed attempts can still receive metadata-only rejection correlation.

## 4. Validation architecture

  Internal harness responsibilities remain separate:

  ToolRegistry
    registration, exact lookup, compiled-schema storage

  ActionProposalDecoder
    response shape and untrusted JSON limits

  ActionValidator
    exact tool lookup and compiled schema instance validation

  CapabilityPolicy
    authorization only

  ExecutionHarness
    checkpoints, audit, correlation, budgets and execution

  Public governed flow:

  pub struct CompletedModelInvocation { /*no Debug*/ }
...
- Existing invoke_model remains source-compatible and delegates to invoke_model_tracked, discarding only the returned correlation metadata.
- prepare_action owns decoder, registry access, validation, checkpoints, and action audit events.
- invoke_validated_action creates a fresh ToolCallId, records binding, constructs the ToolCall, then delegates to the existing invoke_tool.
- No registry, policy, port, schema validator, or RunContext access is exposed through LoopEffects.

## 5. JSON/schema strategy

  Actual JSON Schema validation is required; current ToolSchema only verifies that the schema itself is an object.
  ...
  jsonschema 0.49.2 provides reusable validators and explicit Draft 2020-12 construction. Default features enable HTTP/file resolution, so disabling them prevents schema-triggered retrieval and avoids adding
  another network surface. jsonschema validation API (<https://docs.rs/jsonschema/latest/jsonschema/>), jsonschema feature set (<https://docs.rs/crate/jsonschema/latest/features>)

  M5 supported schema profile:

- Draft 2020-12 only; $schema may be absent or exactly the Draft 2020-12 URI.
- Root type must be "object".
- Supported keywords:
  - Common: type, enum, const
  - Objects: properties, required, additionalProperties as boolean, minProperties, maxProperties
  - Arrays: items, minItems, maxItems
  - Strings: minLength, maxLength
  - Numbers: minimum, maximum

- type unions are rejected.
- $ref, external references, formats, patterns, composition keywords, conditional keywords, custom keywords, and all unknown keywords are rejected.
- Tool schemas are checked against this profile and compiled once during ToolRegistry::register.
- Invalid and unsupported schemas fail registration with sanitized ToolRegistryError variants; they cannot enter a running harness.
- ActionValidator uses the compiled validator and maps all detailed library diagnostics to ArgumentsSchemaMismatch.

## 6. Loop integration

  LoopEngine, its state machine, and all six phase signatures remain unchanged.

  LoopEffects gains only governed M5 operations:

  invoke_model_tracked(...)
  prepare_action(...)
  invoke_validated_action(...)

  The concrete working state is payload-bearing and has no derived Debug:

  struct ActionWorkingState {
      planned_action: Option<ValidatedAction>,
      tool_result: Option<ToolResult>,
  }

  Behavior:

- Plan invokes the model, immediately passes the completed invocation into prepare_action, and stores only ValidatedAction.
- The raw model response is consumed and not retained after successful preparation.
- Act uses planned_action.take() so the validated action is consumed once.
- Verify checks typed facts only: a successful correlated ToolResult, or an explicitly handled domain failure.
- Reflect deterministically returns Complete or Fail; model text never controls the outer loop.
...
## 7. Harness interaction
  ...
  invoke_model_tracked
  → model budget and model audit
  → prepare_action checkpoint
  → strict decode
  → ActionProposed audit
  → exact registry lookup
  → schema validation
  → ActionValidated audit
  → Act
  → ActionExecutionBound audit
  → ExecutionHarness::invoke_tool
  → policy
  → tool budget
  → tool audit
  → ToolPort
...
- Structural/schema validation occurs before policy and before tool-budget reservation.
- LocalWrite, ExternalWrite, and Privileged proposals may validate structurally but are denied later by M0ReadOnlyPolicy.
- A denied action consumes no ToolCalls budget.
- invoke_validated_action does not duplicate invoke_tool policy or terminalization logic.
- Cancellation/deadline checks remain harness-owned.
- Audit FailClosed errors stop preparation or binding before execution; FailOpen marks the run degraded and continues.
- Add a harness invariant verifying that ToolResult::call_id() matches the requested ToolCallId; mismatch becomes a sanitized ToolPortError::AdapterFailure.

## 8. Action/tool correlation
...
- CompletedModelInvocation exposes the harness-generated ModelCallId without exposing it through model/provider APIs.
- ActionProposalId is generated immediately before decoding.
- ToolCallId is generated only when Act consumes ValidatedAction.
- A policy-denied or budget-exhausted action can have a bound ToolCallId but remains uninvoked; existing tool events state the outcome.
- ID values need not be deterministic, but event ordering and correlation structure are deterministic.
- The three ID types remain nominally distinct Rust types.

## 9. Audit model

  Advance CURRENT_EVENT_SCHEMA_VERSION from 3 to 4.

  Add metadata-only variants:
  ...
  → ActionExecutionBound
  → ToolInvocationStarted
  → ToolInvocationCompleted / DomainFailed

  Malformed output emits ActionRejected without ActionProposed. Parsed but invalid proposals emit ActionProposed then ActionRejected.

  No event contains arguments, response text, schema bodies, tool results, raw validator errors, prompts, provider values, or credentials. No persistence migration guarantees are introduced.

## 10. Failure model

   Condition                                                                  Stable handling
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   Empty, malformed, fenced, duplicate, missing or unknown envelope fields    ActionValidationError plus ActionRejected; Plan fails as LoopFailureKind::Program { Plan }
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Unsupported model output shape                                             Same safe Plan failure with distinct rejection reason
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Unknown tool                                                               ActionRejected::UnknownTool; no policy call or ToolCalls usage
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Invalid arguments                                                          ActionRejected::ArgumentsSchemaMismatch; no ToolCalls usage
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Invalid/unsupported tool schema                                            Registration-time ToolRegistryError; run cannot start with that registry
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Policy denial                                                              Existing HarnessError::PolicyDenied, ToolPolicyDenied, run failure category Tool; no ToolCalls usage
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Tool budget exhaustion                                                     Existing terminal BudgetExceeded { ToolCalls }
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   ToolPort infrastructure error or call-ID mismatch                          Existing sanitized tool-adapter failure path
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   ToolResult::DomainFailure                                                  Remains a successful port return with typed domain failure; Verify/Reflect decide failure
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Model failure                                                              Existing ModelPort/RunFailureKind::Model path; no proposal generated
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Verification failure                                                       Existing typed VerificationResult::Failed and Reflect::Fail
  ─────────────────────────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────────────
   Cancellation/deadline                                                      Existing authoritative terminal state from ExecutionHarness

  ActionPreparationError distinguishes validation failure from harness/audit/lifecycle failure without adding action cases to generic LoopStepError.
...
- Planning response: 16 KiB UTF-8 bytes
- Tool name: 128 bytes
- Arguments serialized size: 8 KiB
- Arguments nesting depth: 8
- Total argument object keys: 64
- Total array elements: 128
- Total argument JSON nodes: 256
- Tool schema: 32 KiB, depth 16, and 256 total keys at registration

  Limits are enforced iteratively after bounded decoding; response size is checked before parsing. Duplicate detection occurs during deserialization.

  These are fixed M5 safety constants, not user-configurable budget dimensions. Future policy configuration may version them explicitly.

## 12. Deterministic tests

  Add tests covering:

- Valid exact one-action envelope
- Malformed, fenced, trailing, empty and oversized responses
- Missing, unknown and duplicate fields, including duplicates inside arguments
- Multiple/mixed model output parts rejected atomically
- Tool-name length and exact case-sensitive matching
- Depth, key, array, node and argument-size limits
- Unknown tool
- Valid schema accepted and compiled during registration
- Unsupported keyword, $ref, format, invalid Draft, and malformed schema rejected at registration
- Valid arguments accepted; type, required-field and additional-property violations rejected
- Invalid proposal consumes zero ToolCalls
- Policy-denied LocalWrite/ExternalWrite/Privileged action consumes zero ToolCalls
- ReadOnly action executes exactly once
- ActionProposalId and ToolCallId are distinct and freshly generated
- Exact ModelCallId → ActionProposalId → ToolCallId event correlation
- FailClosed and FailOpen behavior for proposed/validated/bound audit events
- Tool-result call-ID mismatch becomes adapter failure
- Tool domain failure remains distinct from infrastructure failure
- Tool-budget exhaustion remains terminal
- Cancellation/deadline remain harness-authoritative around model and tool futures and between preparation/binding boundaries
- Invalid proposal cannot reach Act
- Full six-phase loop completes with one model-proposed ReadOnly action
- ModelCalls/ToolCalls/Iterations usage equals 1/1/1
- No prompt, arguments, schema, result or provider sentinel appears in serialized events/errors
- M5 core/harness/loop sources contain no Rig/OpenAI imports
- Provider crate does not gain tool execution
- Default tests perform no network access

  Implementation verification will run formatting, strict workspace Clippy, all-feature workspace tests, dependency trees, architecture scans, git diff --check, and the deterministic CLI mode only.

## 13. CLI demonstration

  The default fake model returns the exact JSON envelope for:

  get_agent_capabilities

  The registered fake tool is ReadOnly and uses:
  ...
  The fake tool echoes the harness-generated ToolCallId in its typed result.

  Both deterministic and --live-openai-compatible modes instantiate the same ActionProgram; only ModelPort composition differs.
...
- ToolCallId
- tool name
- phase/iteration events
- final RunStatus
- model/tool/iteration usage
- audit-degraded status

## 14. Dependency changes

- Add workspace jsonschema = { version = "=0.49.2", default-features = false }.
- Add it only to agent-harness.
- Promote serde_json from harness dev-dependency to production dependency for bounded decoding and schema instances.
- No new dependencies for core, loop, provider-rig, or CLI.
- No Tokio feature changes are required.
- No new crate is warranted because proposal vocabulary is core domain data while validation, registry metadata, audit and execution governance are already harness responsibilities.

  Dependency direction remains:

  agent-cli
  ├── agent-loop → agent-harness → agent-core
  └── agent-provider-rig → agent-harness + agent-core + Rig

  agent-harness → jsonschema

## 15. Native-tool-call future path

  M5 will not consume the existing ModelOutputPart::ToolCall(ToolCall) because it is already execution-shaped and cannot safely preserve provider correlation.

  A later sub-milestone will:

- Introduce a provider-neutral native action candidate that is not ToolCall.
- Preserve provider-native correlation only in bounded continuation state designed for assistant/tool-result follow-up.
- Generate a fresh runtime ActionProposalId.
- Feed both textual envelopes and native candidates into the same ActionValidator.
- Produce the same opaque ValidatedAction.
- Generate ToolCallId only at governed Act binding.
- Continue executing exclusively through ExecutionHarness::invoke_tool.

  Rig tools and automatic provider tool loops remain disabled.

## 16. Risks and tradeoffs

- Strict text envelopes may reject otherwise understandable model prose; this is intentional fail-safe behavior until structured output is designed.
- The explicit JSON Schema subset limits expressiveness, but prevents silent keyword ignorance and hidden external resolution.
- Returning ModelCallId requires a small additive harness API, while preserving the existing invoke_model API.
- ValidatedAction is an in-process proof token, not a persistence, sandbox, or cross-process authorization credential.
- Audit schema version 4 intentionally has no backward migration guarantee yet.
- Schema compilation increases registry construction work but removes repeated compilation from action execution.
- ActionExecutionBound precedes policy so denials retain complete correlation; it does not indicate authorization or invocation.
- Native provider correlation, continuation messages, parallel actions, approvals, persistence and write capabilities require later explicit contracts and are deliberately deferred.

# M5 OpenAI web review approvals

The M5 architecture proposal is approved with the following required
adjustments.

Implement M5 after incorporating these decisions.

1. Correct strict JSON envelope semantics.

At the protocol envelope level require exactly:
...
- fuzzy/case-normalized tool names

Inside arguments:

- arguments must be a JSON object
- duplicate keys are always rejected recursively
- security complexity limits are always enforced
- DO NOT reject fields merely because the decoder does not know their names

Whether argument properties are allowed is determined by the registered tool
JSON Schema.

For example:
additionalProperties=false may reject unknown keys.
additionalProperties=true may permit them.

Do not let the envelope decoder override schema semantics.

1. Duplicate detection must happen during deserialization.

Do not first deserialize into serde_json::Value and then attempt to detect
duplicates.

Use a strict custom visitor/deserializer path capable of rejecting duplicate
object keys recursively.

Only after uniqueness is established may the decoded arguments become the
bounded ToolInput representation.

1. Keep parser and validator as separate internal components.

Inside agent-harness/action.rs maintain conceptual separation:

TextActionDecoder
    ModelResponse -> ActionProposal

ActionValidator
    ActionProposal -> ValidatedAction

ExecutionHarness::prepare_action is the governed facade orchestrating them.

Do not make textual JSON-envelope semantics inseparable from schema/action
validation.

This is required so future native provider action candidates can map to
ActionProposal and reuse the same ActionValidator without using
TextActionDecoder.

No new crate is required in M5.

1. Keep ActionProposalId distinct from ToolCallId.

ActionProposalId is generated by trusted runtime code and is never accepted
from the model.

It may be allocated before decoding so malformed attempts have rejection
correlation.

ActionProposal is not:

- executable
- authorized
- a ToolCall

ToolCallId is generated only during governed Act binding.

1. Keep ValidatedAction in agent-harness.
...
- not implement Clone
- not expose raw arguments through public accessors
- be consumed by value during invoke_validated_action

Treat it as an in-process proof of successful structural/schema validation,
not as an authorization credential.

CapabilityPolicy still runs afterwards.

1. Keep CompletedModelInvocation opaque for the action path.

invoke_model_tracked returns CompletedModelInvocation containing the
harness-generated ModelCallId and provider-neutral ModelResponse.

prepare_action consumes it.

Do not expose a raw tracked-response accessor merely for convenience unless an
actual non-action use case requires it.

Existing invoke_model remains available for callers that simply need a
ModelResponse.

1. JSON Schema dependency.

Use the inspected/pinned jsonschema version only after confirming its exact
0.49.2 API during implementation.
...
Promote serde_json to an agent-harness production dependency as required.

Do not enable network/file reference resolution features.

1. Force Draft 2020-12 explicitly.

Do not rely on automatic draft detection.

Compile accepted schemas using the Draft 2020-12-specific validator builder.

Schema policy:

- $schema absent -> interpret as Draft 2020-12
- $schema exactly the approved Draft 2020-12 URI -> accept
- another $schema -> reject registration

$ref remains unsupported in M5.

1. Schema profile checking must be semantic/context-aware.

Do NOT recursively treat every JSON object key as a schema keyword.

Correctly distinguish:

schema-valued locations:

- root schema
- properties values
- items

from data-valued locations:

- enum values
- const values

Property names under "properties" are arbitrary tool argument names and must
not be checked as JSON Schema keywords.

Implement profile validation deliberately rather than a generic recursive
"all keys must be known keywords" walk.

1. Supported M5 schema profile.

Keep the proposed validation subset, but also allow harmless annotation
keywords:
...
- dependentSchemas
- unknown/custom keywords

If jsonschema compilation or the selected profile reveals a semantic issue,
fail tool registration safely rather than silently weakening validation.

1. Compile schemas once at registration.

ToolRegistry continues to snapshot ToolPort::definition().

Validate:

- schema byte-size limit
- schema complexity/profile
- draft policy

then compile once and retain the compiled validator with the binding.

If the jsonschema Validator type is not cheaply cloneable, store it behind an
appropriate immutable shared container rather than recompiling per action.

Registry still performs no authorization.

1. Validation sequence remains:

model response
→ strict decoder
→ ActionProposal
→ ActionProposed metadata
→ exact ToolRegistry lookup
→ argument JSON Schema validation
→ ActionValidated metadata
→ ValidatedAction

Invalid structural/schema proposal:

- no CapabilityPolicy execution
- no ToolCalls budget
- no ToolPort call

1. Policy/execution sequence remains:

ValidatedAction
→ Act consumes value
→ generate fresh ToolCallId
→ record proposal/tool binding metadata
→ ExecutionHarness::invoke_tool
→ CapabilityPolicy
→ tool budget
→ audit
...
ActionExecutionBound (or renamed ActionToolCallBound) means only:

"this proposal has been bound to a runtime ToolCall identity."
...
A policy-denied action may therefore have a ToolCallId for correlation while
remaining uninvoked and consuming zero ToolCalls.

1. Tool result correlation.

Add ToolResult::call_id() or equivalent provider-neutral correlation access.

After ToolPort returns, ExecutionHarness must verify that returned ToolCallId
matches the requested ToolCallId.

Mismatch is a sanitized adapter/infrastructure failure, not a domain-level
ToolResult failure.

Ensure all successful and domain-failure ToolResult variants preserve their
associated ToolCallId consistently.

1. Security limits.

Keep the proposed fixed M5 limits:
...
Check planning response size BEFORE JSON parsing.

Enforce structural limits before schema validation.

These are fixed defensive M5 invariants, not RunBudget dimensions.

1. Audit events.

Advance schema version to 4 as proposed.
...
ActionValidated
ActionRejected
ActionExecutionBound / ActionToolCallBound
...
Malformed attempts may emit ActionRejected without ActionProposed.

Parsed but invalid proposals emit ActionProposed followed by ActionRejected.

1. ReadOnly execution only.

Do not change M0ReadOnlyPolicy.

ReadOnly validated actions may execute.

LocalWrite / ExternalWrite / Privileged proposals may parse and validate but
must still be denied by CapabilityPolicy.

No approval mechanism in M5.

Policy denial consumes zero ToolCalls.

1. Preserve deterministic loop authority.

LoopEngine and six-phase transitions should remain unchanged.

Concrete ActionProgram:
...
→ Plan:
     invoke_model_tracked
     prepare_action
     store only ValidatedAction
→ Act:
     consume ValidatedAction once
     invoke_validated_action
→ Verify:
     inspect typed ToolResult/correlation/domain status only
→ Reflect:
     deterministic Complete/Fail

Free-form model text must never directly determine Continue/Complete/Fail.

1. Live OpenAI-compatible mode.

Use the same ActionProgram for:

- deterministic FakeRigModel
- M4 --live-openai-compatible ModelPort

Only model composition changes.

The live model may fail to obey the strict envelope; this is a safe action
preparation failure, not a reason to add markdown/prose extraction heuristics.

Do not require live network tests in CI.

1. No native provider tool execution.

Do not map ModelOutputPart::ToolCall in M5.

Do not enable Rig tools or OpenAI tool definitions.

Native tool-call support remains a future adapter path:

native candidate
→ provider-neutral ActionProposal
→ same ActionValidator
→ same ValidatedAction
→ same governed Act path

1. Tests must include all proposed tests plus:

- arguments with an unknown property are accepted when schema permits
  additionalProperties=true
- the same unknown property is rejected when additionalProperties=false
- property names under schema "properties" are not treated as schema keywords
- object values inside enum/const are not traversed as schema syntax
- nested duplicate argument keys are rejected during decoding
- duplicate detection occurs before ToolInput construction
- Draft 2020-12 is selected explicitly
- wrong $schema is rejected
- title/description annotations are accepted
- ActionExecutionBound does not imply policy authorization
- policy-denied action may correlate ProposalId -> ToolCallId but ToolPort
  invocation count remains zero
- ToolResult call-id mismatch becomes adapter failure
- successful/domain-failure ToolResults preserve exact call ID
- live-style textual response containing code fences is safely rejected

1. Architecture boundaries.

No Rig/OpenAI/provider concerns may enter:

- agent-core
- agent-harness action domain beyond provider-neutral ModelResponse
- agent-loop

agent-provider-rig remains model/provider integration only and must gain no
tool execution.
...
Run architecture scans proving:

- no Rig/OpenAI imports in core/harness/loop
- provider-rig contains no tool execution path

Run deterministic CLI mode only.
Do not automatically execute external live mode.

1. Final implementation report:

1. changed tree
1. action domain types
1. strict decoder behavior
1. duplicate detection
1. security-limit enforcement
1. JSON Schema profile and compilation
1. ValidatedAction invariants
1. tracked ModelCall correlation
1. proposal/tool-call binding
1. policy/budget behavior
1. ToolResult call-ID invariant
1. audit events
1. deterministic ActionProgram
1. live-mode compatibility
1. dependency changes
1. architecture-boundary results
1. tests/quality gates
1. deliberate deferrals
1. deviations and justification
